...
- Use-case регистрации принимает ровно один из `email` / `login`. Инвариант сохранённого аккаунта: задан **минимум один** идентификатор; после подтверждённой привязки могут быть заданы оба.
...
**Подтверждение email и привязка идентификаторов:**

- Email-регистрация создаёт неподтверждённый аккаунт и verification token TTL 24 часа. До подтверждения email нельзя использовать для обычного входа и recovery; разрешены только повторная отправка письма и подтверждение.
- Неподтверждённые email-аккаунты удаляются cleanup-job через 24 часа, если подтверждение не завершено.
- Привязка email к login-аккаунту требует свежей проверки текущего пароля (`recent re-auth`, не старше 10 минут). Адрес хранится в verification token как pending и переносится в `users.email` только после подтверждения.
- Привязка login к email-аккаунту также требует recent re-auth. Уникальность проверяется транзакционно.
- Verification/reset token одноразовый, в БД только хеш. Повторная отправка инвалидирует предыдущие активные токены того же назначения.
...
3. Если пользователь с подтверждённым email есть — Hangfire/SMTP шлёт письмо с deep link и одноразовым токеном TTL 30 минут.
4. Экран смены пароля по токену: `POST /api/v1/auth/reset-password`.
5. Rate limit на forgot-password (Redis).
6. Reset выполняется одной транзакцией: токен атомарно помечается использованным, меняется password hash, инвалидируются остальные reset-токены и **все** refresh-токены пользователя.
7. Аккаунт, зарегистрированный только по логину, нельзя восстановить, пока email не привязан и не подтверждён.
...
Регистрация принимает ровно один идентификатор; DB требует минимум один, после verification могут быть оба.
...
- Rate limit обязателен для login, registration, verification resend, forgot/reset, refresh, bind identifier, URL issue, upload и admin import; точные ключи и значения — в API contract.
...
- Регистрация принимает ровно email или login. Email-вход/recovery недоступны до verification; привязка email требует recent re-auth и подтверждения.
...
Инструкции по локальному запуску поддерживаются в `README` корня репозитория по мере появления кода.
