...
- Email-регистрация создаёт аккаунт с login+email и одноразовый **6-значный код** в письме (TTL 24 часа, в БД только хеш). Deep link не используется. До подтверждения email нельзя использовать для входа по почте и recovery; разрешены повторная отправка письма и ввод кода в приложении. Вход по login доступен сразу.
