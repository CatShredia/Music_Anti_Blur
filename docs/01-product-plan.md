...
- Email-регистрация создаёт аккаунт с login+email и verification token TTL 24 часа. До подтверждения email нельзя использовать для входа по почте и recovery; разрешены повторная отправка письма и подтверждение. Вход по login доступен сразу.
